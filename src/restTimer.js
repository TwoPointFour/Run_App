...
export function initialiseTimer(input) {
  ...
  const permSetTime = 50000000000;
  const permPaceCount = permDistance / 100;
  const permRestTime = permRestTimeSec * 1000;
...
function activateSplitBtn(variablePackage) {
  document.querySelector(".timersplit").addEventListener("click", function () {
    ...
    variablePackage.setTime = variablePackage.permRestTime;
    variablePackage.splitToggle = true;
  });
}
...
function displayMinutes(variablePackage) {
  let minutes;
  let seconds;
  if (!variablePackage.splitToggle) {
    [minutes, seconds] = toMinutesSeconds(variablePackage.permSetTime - variablePackage.setTime);
  } else {
    [minutes, seconds] = toMinutesSeconds(variablePackage.setTime);
  }
...
function updateSetTimer(variablePackage) {
  if (variablePackage.setTime <= 0 && variablePackage.setCount < variablePackage.permSetCount) {
    ...
    variablePackage.splitToggle = false;
  ...
  displayMinutes(variablePackage);
