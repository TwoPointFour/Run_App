...
document.querySelector(".distancesubmit").addEventListener("click", function () {
  ...
  permdistrepetition = Number(document.querySelector("#distrepetition").value);
  permdistrepcount = 1;
  permpacecount = 1;
...
updateCountdown = function () {
  ...
  if (pacecount >= numdistset && distpace <= 0) {
    ...
    document.querySelector(".timermin").textContent = `${setminutes}:${setsecondscut}`;
  }
  ...
  document.querySelector(".milliten").textContent = `${pacemillicut.toString().slice(0, 1)}`;
  // document.querySelector(
  //   ".millione"
...
document.querySelector(".timersplit").addEventListener("click", function () {
  ...
  const splitseconds = Math.floor((splittotal - splitminutes * 1000 * 60) / 1000);
  const splitmilli = splittotal - splitminutes * 1000 * 60 - splitseconds * 1000;

  document.querySelector(".timerecord").innerHTML += `
  <tr>
    <th scope="row">${distrepcount}</th>
    <td>${`${addzerosec(splitminutes)}:${addzerosec(splitseconds)}:${addzero(splitmilli)}`}</td>
